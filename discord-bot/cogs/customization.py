...
async def _build_event_embed(
    ...
    defaults = {
        ...
        "poj":     "{user.mention}",
        "doj":     "Welcome to **{server}**, {user}! Glad to have you here.",
    }
...
class Customization(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # Ping-on-join: short ping in a channel, auto-deletes after a few seconds.
        if config.get("poj_enabled") and config.get("poj_channel"):
            ch = member.guild.get_channel(config["poj_channel"])
            if isinstance(ch, discord.TextChannel):
                content, embed, _ = await _build_event_embed(config, member, self.ef, "poj", ch)
                # POJ is meant to be a lightweight ping — embed-less unless explicitly linked.
                if not config.get("poj_embed") and "{embed:" not in (config.get("poj_message") or ""):
                    embed = None
                if content or embed:
                    raw = config.get("poj_delete_after")
                    delete_after = float(raw) if raw is not None else 5.0
                    try:
                        await ch.send(
                            content=content,
                            embed=embed,
        ...
        # DM-on-join: send a personal welcome DM to the new member.
        if config.get("doj_enabled"):
            content, embed, _ = await _build_event_embed(config, member, self.ef, "doj", None)
            try:
    ...
    poj_group = app_commands.Group(name="poj", description="Ping-on-join settings.", parent=set_group)

    @poj_group.command(name="message", description="Set the ping-on-join message template.")
    @app_commands.describe(message="Template with {variables}. Default: {user.mention}")
    async def poj_message(self, interaction: discord.Interaction, message: str):
        assert interaction.guild
        await upsert_guild_config(interaction.guild.id, poj_message=message)
        await interaction.response.send_message(
            embed=self.ef.success(f"POJ message updated.\n```{message[:200]}```"), ephemeral=False
        )

    @poj_group.command(name="channel", description="Set the channel where new members are pinged.")
    @app_commands.describe(channel="Channel to ping new members in.")
    async def poj_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        assert interaction.guild
        await upsert_guild_config(interaction.guild.id, poj_channel=channel.id, poj_enabled=1)
        await interaction.response.send_message(
            embed=self.ef.success(f"Ping-on-join will fire in {channel.mention}."), ephemeral=False
        )

    @poj_group.command(name="toggle", description="Enable or disable ping-on-join.")
    async def poj_toggle(self, interaction: discord.Interaction):
        assert interaction.guild
        config = await get_guild_config(interaction.guild.id)
        new = 0 if config.get("poj_enabled") else 1
        await upsert_guild_config(interaction.guild.id, poj_enabled=new)
        await interaction.response.send_message(
            embed=self.ef.success(f"Ping-on-join {self.ef.e['enabled'] if new else self.ef.e['disabled']} **{'enabled' if new else 'disabled'}**."), ephemeral=False
        )

    @poj_group.command(name="embed", description="Link a named embed to ping-on-join messages.")
    @app_commands.describe(name="Name of the saved embed to attach (use 'none' to unlink).")
    async def poj_embed(self, interaction: discord.Interaction, name: str):
        assert interaction.guild
        clean = name.lower().strip()
        if clean in ("none", "off", "clear", "remove"):
            await upsert_guild_config(interaction.guild.id, poj_embed=None)
            return await interaction.response.send_message(
                embed=self.ef.success("Embed unlinked from ping-on-join."), ephemeral=False
            )
        existing = await get_embed(interaction.guild.id, clean)
        if not existing:
            return await interaction.response.send_message(
                embed=self.ef.error(f"No embed named `{clean}` found. Create one with `/embed create`."), ephemeral=False
            )
        await upsert_guild_config(interaction.guild.id, poj_embed=clean)
        await interaction.response.send_message(
            embed=self.ef.success(f"Ping-on-join will now use `{{embed:{clean}}}`."), ephemeral=False
        )

    @poj_group.command(name="deleteafter", description="Auto-delete the ping after N seconds (0 = never).")
    @app_commands.describe(seconds="Seconds before the ping is deleted. Default 5, max 60.")
    async def poj_deleteafter(self, interaction: discord.Interaction, seconds: app_commands.Range[int, 0, 60]):
        assert interaction.guild
        await upsert_guild_config(interaction.guild.id, poj_delete_after=int(seconds))
        msg = "Ping will not auto-delete." if seconds == 0 else f"Ping will be deleted after **{seconds}s**."
        await interaction.response.send_message(embed=self.ef.success(msg), ephemeral=False)

    doj_group = app_commands.Group(name="doj", description="DM-on-join settings.", parent=set_group)

    @doj_group.command(name="message", description="Set the DM-on-join message template.")
    @app_commands.describe(message="Template with {variables} and optional {embed:name}.")
    async def doj_message(self, interaction: discord.Interaction, message: str):
        assert interaction.guild
        await upsert_guild_config(interaction.guild.id, doj_message=message)
        await interaction.response.send_message(
            embed=self.ef.success(f"DOJ message updated.\n```{message[:200]}```"), ephemeral=False
        )

    @doj_group.command(name="toggle", description="Enable or disable DM-on-join.")
    async def doj_toggle(self, interaction: discord.Interaction):
        assert interaction.guild
        config = await get_guild_config(interaction.guild.id)
        new = 0 if config.get("doj_enabled") else 1
        await upsert_guild_config(interaction.guild.id, doj_enabled=new)
        await interaction.response.send_message(
            embed=self.ef.success(f"DM-on-join {self.ef.e['enabled'] if new else self.ef.e['disabled']} **{'enabled' if new else 'disabled'}**."), ephemeral=False
        )

    @doj_group.command(name="embed", description="Link a named embed to DM-on-join messages.")
    @app_commands.describe(name="Name of the saved embed to attach (use 'none' to unlink).")
    async def doj_embed(self, interaction: discord.Interaction, name: str):
        assert interaction.guild
        clean = name.lower().strip()
        if clean in ("none", "off", "clear", "remove"):
            await upsert_guild_config(interaction.guild.id, doj_embed=None)
            return await interaction.response.send_message(
                embed=self.ef.success("Embed unlinked from DM-on-join."), ephemeral=False
            )
        existing = await get_embed(interaction.guild.id, clean)
        if not existing:
            return await interaction.response.send_message(
        ...
        await upsert_guild_config(interaction.guild.id, doj_embed=clean)
        await interaction.response.send_message(
            embed=self.ef.success(f"DM-on-join will now use `{{embed:{clean}}}`."), ephemeral=False
        )
    ...
    @test_group.command(name="poj", description="Sends a test ping-on-join in the configured POJ channel.")
    async def test_poj(self, interaction: discord.Interaction):
        assert interaction.guild and isinstance(interaction.user, discord.Member)
        await interaction.response.defer(ephemeral=False)
        config = await get_guild_config(interaction.guild.id)
        ch_id = config.get("poj_channel")
        if not ch_id:
            return await interaction.followup.send(
                embed=self.ef.error("No POJ channel set. Use `/set poj channel` first."), ephemeral=False
            )
        ch = interaction.guild.get_channel(ch_id)
        if not isinstance(ch, discord.TextChannel):
            return await interaction.followup.send(
                embed=self.ef.error("POJ channel not found or is not a text channel."), ephemeral=False
            )
        content, embed, _ = await _build_event_embed(config, interaction.user, self.ef, "poj", ch)
        if not config.get("poj_embed") and "{embed:" not in (config.get("poj_message") or ""):
            embed = None
        if not (content or embed):
            return await interaction.followup.send(
                embed=self.ef.error("POJ has no message or embed to send."), ephemeral=False
            )
        raw = config.get("poj_delete_after")
        delete_after = float(raw) if raw is not None else 5.0
        try:
            ...
            await interaction.followup.send(embed=self.ef.success(f"Test POJ sent to {ch.mention}."), ephemeral=False)
        except discord.Forbidden:
            await interaction.followup.send(embed=self.ef.error(f"No permission to send in {ch.mention}."), ephemeral=False)
        except discord.HTTPException as e:
            await interaction.followup.send(embed=self.ef.error(f"Failed to send: {e}"), ephemeral=False)

    @test_group.command(name="doj", description="DMs you a test DM-on-join message.")
    async def test_doj(self, interaction: discord.Interaction):
        assert interaction.guild and isinstance(interaction.user, discord.Member)
        await interaction.response.defer(ephemeral=True)
        config = await get_guild_config(interaction.guild.id)
        content, embed, _ = await _build_event_embed(config, interaction.user, self.ef, "doj", None)
        try:
            await interaction.user.send(content=content, embed=embed)
            await interaction.followup.send(embed=self.ef.success("Test DOJ sent to your DMs."), ephemeral=True)
        except (discord.Forbidden, discord.HTTPException):
            await interaction.followup.send(
                embed=self.ef.error("Could not DM you — your DMs may be closed for this server."),
                ephemeral=True,
            )
