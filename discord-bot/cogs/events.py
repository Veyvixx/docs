...
class Events(commands.Cog):
    ...
    async def _ch(self, guild: discord.Guild) -> Optional[discord.TextChannel]:
        """
        Return the configured server-event log channel.
        Prefers the dedicated `log_channel_id` (set via /log_channel) but falls
        back to `mod_log_channel` (set via /modlog) so users who only configure
        one channel still get all events logged there.
        """
        config = await get_guild_config(guild.id)
        ch_id  = config.get("log_channel_id") or config.get("mod_log_channel")
        if not ch_id:
            return None
