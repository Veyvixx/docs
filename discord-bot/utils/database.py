...
async def set_premium(
    ...
    expr, names, values = _build_update(fields)
    # Stamp created_at on first insert via if_not_exists — must live in the SET
    # clause, so splice it in before any REMOVE clause to keep the expression valid.
    names["#ca"] = "created_at"
    values[":cav"] = int(time.time())
    ca_set = "#ca = if_not_exists(#ca, :cav)"
    if expr.startswith("SET "):
        # Insert into existing SET clause (before any REMOVE that follows).
        if " REMOVE " in expr:
            head, _, tail = expr.partition(" REMOVE ")
            expr = f"{head}, {ca_set} REMOVE {tail}"
        else:
            expr = f"{expr}, {ca_set}"
    else:
        # No SET clause yet (only REMOVEs) — prepend one.
        expr = f"SET {ca_set} " + expr
